...
void SendToEco(uint16_t data)
{

  uint8_t mbox;	 
	CanTxMsg TxMessage;
	
	memset(TxMessage.Data,0,sizeof(uint8_t)*8);
	TxMessage.StdId=0x66;					 // standard identifier=0
	TxMessage.ExtId=0x66;					 // extended identifier=StdId
	TxMessage.IDE=CAN_Id_Standard ;			 // type of identifier for the message is Standard
	TxMessage.RTR=CAN_RTR_Data  ;			 // the type of frame for the message that will be transmitted
	TxMessage.DLC=8;
  ...
  txdata.u16_data=data;
	TxMessage.Data[0] =txdata.data[0];
	TxMessage.Data[1] =txdata.data[1];
